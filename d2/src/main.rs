...
fn main() {
    ...
    let mut submarine = SubMarine{ depth: 0, horizontal: 0};

    for x in input {
        match x {
            Movement::Down(x)=> {submarine.depth= submarine.depth+x;},
            Movement::Up(x)=> {submarine.depth= submarine.depth-x;},
            Movement::Forward(x)=> {submarine.horizontal = submarine.horizontal+x;},
            _ => continue
        }
    }
